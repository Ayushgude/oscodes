...
void run(int n, int tq) {
    ...
    while(!tempGanttTimes.empty()) {
        if(tempRunningq.front()!=-1) {
            ct[tempRunningq.front()]=tempGanttTimes.front();

            if(responset[tempRunningq.front()]>(tempGanttTimes.front() - at[tempRunningq.front()])) {
                responset[tempRunningq.front()]=tempGanttTimes.front() - at[tempRunningq.front()];
            }
        }
